...
# extract .zip-packaged tools (stylua, omnisharp). python3(+venv+pip) lets Mason
# install debugpy — the real image already has these; the slim base doesn't.
apt-get install -y --no-install-recommends curl ca-certificates git tar gzip unzip gcc g++ make libc6-dev python3 python3-venv python3-pip >/dev/null
...
timeout 2400 nvim --headless -c "luafile $P/prime.lua"
